...
#        self.aligned_points_bbox = None                     #: Max bounding box for all aligned shapes
        self._datadir = None
...
#        self.aligned_points_bbox = Rect(int(round(np.min(points_x))), int(round(np.max(points_x))), int(round(np.min(points_y))), int(round(np.max(points_y))))
