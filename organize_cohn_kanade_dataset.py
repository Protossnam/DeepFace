...
def main(argv):
    parser = argparse.ArgumentParser(description='Cohn-Kanade dataset organizer : this program reads the original\
                                     Cohn-Kanade dataset from an input folder containing "cohn-kanade-images", "Landmarks" and "Emotion" subfolders and saves\
    ...
    assert os.path.exists(img_dir), 'Cannot find "cohn-kanade-images" subfolder'
...
#                print(emotion_dir.name)
                ...
                for k, img_path in enumerate(selected_files):
                    ...
                    landmarks_path = landmarks_path.replace('cohn-kanade-images', 'Landmarks').replace('.png', '_landmarks.txt')
                    emotion_path = os.path.abspath(img_path)
                    emotion_path = emotion_path.replace('cohn-kanade-images', 'Emotion').replace('.png', '_emotion.txt')
